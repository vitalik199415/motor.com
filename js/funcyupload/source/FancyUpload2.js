...
(function() {
	var phrases = {
		'progressOverall': 'Общий размер файлов ({total})',
		'currentTitle': 'Текущий файл',
		'currentFile': 'Загружается "{name}"',
		'currentProgress': 'Загружено: {bytesLoaded} Скорость {rate}, {timeRemaining} Примерно осталось.',
		'fileName': '{name}',
		'remove': 'Удалить',
		'removeTitle': 'Кликните для удаления',
		'fileError': 'Загрузка остановлена',
		'validationErrors': {
			'duplicate': 'Файл <strong>{name}</strong> уже был загружен.',
